...
class RunnerM():
    ...
    def train(self, train_set, dev_set, **kwargs):
        ...
        for epoch in range(num_epochs):
            ...
            for iteration in tqdm(range(int(X.shape[0] / self.batch_size) + 1), desc=f"Processing epoch {i}", unit="iter"):
                ...
                if self.early_stopping is not None:
                        self.early_stopping(self.model, dev_loss)
                        if self.early_stopping.early_stop:
                            print(f"Early stopping triggered at epoch {epoch}, iteration {iteration}")
                            # 恢复最佳权重
                            if self.early_stopping.restore_best_weights:
                                print("Restoring best model weights")
                            return
            # 在runner.py的train()方法中添加
            history = {
                'train_acc': self.train_scores,
                'val_acc': self.dev_scores,
                'train_loss': self.train_loss,
                'val_loss': self.dev_loss
            }
            with open(os.path.join(save_dir, 'training_history.pickle'), 'wb') as f:
